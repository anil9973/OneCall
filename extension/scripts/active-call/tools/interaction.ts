...
export class InteractionTools {
	...
	constructor(domExtractor: DOMExtractor) {
		this.domExtractor = domExtractor;
	}

	/** Get interactive elements (buttons, links, inputs) */
	async getInteractiveElements(params: { type?: "click" | "input" | "select" | "all" }) {
		const { type = "all" } = params;
		const stepType = type === "all" ? "generic" : type;
		const elements = await this.domExtractor.extractInteractiveElements(stepType as any);
	...
	async findElement(params: { eId?: string; indexPath?: number[] }): Promise<HTMLElement | undefined> {
		const { eId, indexPath } = params;
		const element = eId
			? this.domExtractor.getElementByEId(eId)
			: indexPath
			? this.domExtractor.getElementByIndexPath(indexPath)
			: null;
	...
	async highlightElement(params: { eId?: string; indexPath?: number[] }) {
	...
	/** Click element */
	async clickElement(params: { eId?: string; indexPath?: number[]; doubleClick?: boolean }) {
		try {
			const element = await this.findElement(params);
			if (!element) return { success: false, error: "Element not found" };
	...
	/** Fill input field */
	async fillInputField(params: { eId?: string; indexPath?: number[]; value: string; clearFirst?: boolean }) {
		try {
			const element = await this.findElement(params);
			if (!element) return { success: false, error: "Element not found" };
			if (!["INPUT", "TEXTAREA"].includes(element.tagName)) return { success: false, error: "Not an input element" };

			const inputElement = element as HTMLInputElement | HTMLTextAreaElement;
			if (params.clearFirst) inputElement.value = "";
	...
	async selectDropdownOption(params: {
		...
		try {
			const element = await this.findElement(params);
			if (!element) return { success: false, error: "Element not found" };
			if (!element || element.tagName !== "SELECT") return { success: false, error: "Not a select element" };
	...
	/** Scroll page */
	async scrollPage(params: { direction?: "up" | "down" | "top" | "bottom"; pixels?: number; toElement?: string }) {
	 ...
	 */
	private showGuidedAction(targetElement: Element) {
		...
		// 7. Inject CSS if not already present
		this.injectStyles();

		// 8. Update Position Logic
		const updatePosition = () => {
			const rect = targetElement.getBoundingClientRect();
			const scrollTop = window.scrollY || document.documentElement.scrollTop;
			const scrollLeft = window.scrollX || document.documentElement.scrollLeft;

			// Position the spotlight exactly over the element
			...
			// Position hand slightly below and right of the element
		...
		// Initial position + listener for scroll/resize
		updatePosition();
		window.addEventListener("resize", updatePosition);
		window.addEventListener("scroll", updatePosition);

		// 9. Auto-remove on click
		const cleanup = () => {
			guideLayer?.remove();
		...
		// Allow the user to click the element to dismiss
		targetElement.addEventListener("click", cleanup);
		// Or dismiss if they click the background (optional)
	...
	injectStyles() {
		if (document.getElementById("onecall-guidance-styles")) return;
