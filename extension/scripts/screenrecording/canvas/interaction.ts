...
import type { Point, ToolType, Shape } from "./types.js";
...
export class InteractionManager {
	...
	private lastAddedPointTime = 0;
	private readonly POINT_THROTTLE_MS = 16; // ~60fps
	...
	constructor(
		...
		private onTextInput: (x: number, y: number, callback: (text: string) => void) => void,
		private getActiveTool: () => ToolType,
		private getActiveStyle: () => ShapeStyle,
		private getSelectedShapeId: () => string | null,
		private setSelectedShapeId: (id: string | null) => void
	) {
		this.attachEvents();
	}
	...
	private handleSelectionStart(point: Point): void {
	...
	private handleTextToolClick(point: Point): void {
		this.onTextInput(point.x, point.y, (text: string) => {
			if (!text.trim()) return;
	...
	private handleDrawingStart(point: Point, tool: ToolType): void {
		this.isDrawing = true;
	...
	private handlePenMove(point: Point): void {
		const now = Date.now();
	...
	private handleShapeUpdate(point: Point, tool: ToolType): void {
	...
	private finalizeDrag(): void {
		...
		if (Math.abs(dx) > 1 || Math.abs(dy) > 1) {
			// Undo the move, then do it via command
			this.draggedShape.move(-dx, -dy);
	...
	private finalizeDrawing(): void {
		...
		// For very small shapes, remove them
