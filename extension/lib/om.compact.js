...
var updator = {
  ...
  update(reactData) {
    reactData.cmtNode ? updator.setSibling(reactData) : reactData.func ? reactData.func() : updator.setValue(reactData);
  }
};
var reactMaker = {
...
function reactive(targetObj) {
  ...
  Object.defineProperty(targetObj, "$on", {
    value: (prop, func) => reactMaker.insertReactData(targetObj, prop, { func }),
    enumerable: false
  });
  ...
  const modifiers = {
    push: () => function(...items) {
      ...
      for (const reactData of targetArray["_react"]) {
        const childItems = getItems(items, reactData.yieldfn), parentElem = reactData.comment.parentElement;
    ...
    filter: () => function() {
      ...
      const func = arguments[0], reactData = targetArray["_react"][0], childNodes = reactData.comment.parentNode.children;
...
var fragment = {
  ...
  insertReactChildFrag(cmtNode, frag) {
    ...
    if (siblingFrag instanceof Promise) siblingFrag.then((docFrag) => this.replaceChildFrag(cmtNode, docFrag));
      ...
      Array.isArray(siblingFrag) ? react.cmt.after(...siblingFrag) : react.cmt.after(siblingFrag ?? new Comment(String(Math.random()).slice(9)));
  ...
  setChildFragment(domFrag) {
    const nodeIterator = document.createNodeIterator(
      ...
      (node) => node.nodeValue?.startsWith(" ?") ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT
    );
    let cmtNode;
    while (cmtNode = nodeIterator.nextNode()) {
      ...
      if (frag) {
        if (frag instanceof Function) {
          cmtNode.nodeValue.startsWith(" ?^") ? this.insertReactChildFrag(cmtNode, frag) : this.insertReactTxtCnt(cmtNode, frag);
  ...
  setPromiseFrag(cmtName, docFrag, rootElem) {
    const nodeIterator = document.createNodeIterator(
      document.body,
      NodeFilter.SHOW_COMMENT,
      (node) => node.nodeValue === cmtName ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT
    );
...
var attrParser = {
  ...
  ".": (element, attr, attrMap) => {
    ...
    reactAttrs.has(attr.value) ? attrParser.rtvAttr(element, attr, attrMap) : element[attr.name.slice(1)] = objMap.get(attr.value) ?? attr.value ?? "";
    attrMap.removeNamedItem(attr.name);
    return true;
  },
  /**@param {Element} element, @param {Attr} attr, @param {NamedNodeMap} attrMap*/
  "?": (element, attr, attrMap) => {
    attrParser.mutatNodes[element.tagName] && (element["bindEvent"] = attrParser.mutatNodes[element.tagName]);
    reactAttrs.has(attr.value) ? attrParser.rtvAttr(element, attr, attrMap) : element[attr.name.slice(1)] = attr.value === "true";
    attrMap.removeNamedItem(attr.name);
    return true;
  },
  /**@param {Element} element, @param {Attr} attr, @param {NamedNodeMap} attrMap*/
